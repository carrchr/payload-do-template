...
App Platform's **build** step — buildpack or Dockerfile — never has database access. This is documented DO behavior, not a bug: see their [troubleshooting guide](https://docs.digitalocean.com/support/why-does-my-app-fail-to-build-while-trying-to-connect-to-a-digitalocean-managed-database/), which recommends against connecting to a database during build at all. The stock Payload website template queries Postgres at build time in several places — `generateStaticParams` in `[slug]/page.tsx`, `posts/[slug]/page.tsx`, and `posts/page/[pageNumber]/page.tsx` to pre-render every page/post path, plus static rendering of `/posts` — all of which fail outright on App Platform.

The **run** step, unlike build, does have database access (that's how `pnpm payload migrate` and every dynamic route already work). So instead of disabling static generation outright, the `web` service's build is split across Next's experimental two-phase build mode: `build_command: pnpm build:compile` (`next build --experimental-build-mode compile`, no DB needed) produces a working build, and `run_command: pnpm build:generate && pnpm start` runs the DB-dependent generation pass (`next build --experimental-build-mode generate`) once the container starts — reachable database, before `next start` begins serving traffic — then starts the server. The existing `afterChange` revalidation hooks (`revalidatePage.ts`/`revalidatePost.ts`) keep the now-static pages fresh exactly as they did for the stock template. The two sitemap route handlers stay `force-dynamic` with `unstable_cache` — that dynamic-caching setup doesn't depend on build-time DB access and doesn't need this split.

The trade-off: every deploy now does a full site prerender during container startup, before App Platform's health check can pass, so deploys are slower than a plain `next start` would be — worth watching if the content volume grows large enough to approach DO's deploy timeout. The only build-time (`build_command`) env var this project still needs is `NEXT_PUBLIC_SERVER_URL` (inlined into the client bundle), which uses the bindable `${APP_URL}`.
